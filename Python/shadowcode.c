                     ...
                     PyObject *name)
{
    ...
    /* grow the array if we don't have a free spot */
    if (shadow->globals == NULL || index == -1) {
        index = shadow->globals_size;
        if (!generic_cache_grow((void**)&shadow->globals, &shadow->globals_size, sizeof(GlobalCacheEntry))) {
             return;
         }
     }
                        ...
                        PyObject **res)
{
    ...
    if (mod_dict) {
        int version = PYCACHE_MODULE_VERSION(owner);
        ...
        getattr = _PyDict_GetItemId(((PyModuleObject *)owner)->md_dict,
                                    &PyId___getattr__);
        if (getattr) {
                               ...
                               PyObject **method)
{
    ...
    if (mod_dict) {
        int version = PYCACHE_MODULE_VERSION(obj);
        PyObject *value = PyDict_GetItem(mod_dict, name);
        if (value != NULL) {
            _PyShadow_ModuleAttrEntry *entry =
                (_PyShadow_ModuleAttrEntry *)_PyShadow_NewCacheEntry(
...
_PyShadowCode_Free(_PyShadowCode *shadow)
{
    if (shadow->globals_size) {
        for (Py_ssize_t i = 0; i < shadow->globals_size; i++) {
            Py_XDECREF(shadow->globals[i].name);
        }
